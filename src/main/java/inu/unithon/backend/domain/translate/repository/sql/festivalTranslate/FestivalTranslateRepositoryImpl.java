...
import inu.unithon.backend.domain.translate.entity.QFestivalTranslate;
...
public class FestivalTranslateRepositoryImpl implements FestivalTranslateRepositoryCustom {
  ...
  public boolean existsByContentIdAndLanguage(Long contentId, TranslateLanguage language) {
    Integer fetchOne = queryFactory
      ...
      .from(festivalTranslate)
      .where(
        festivalTranslate.contentId.eq(contentId)
          .and(festivalTranslate.language.eq(language))
      )
      .fetchFirst(); // limit 1 효과

    return fetchOne != null;
  }

  /**
   * contentId와 language로 삭제
   */
  @Override
  public void deleteByContentIdAndLanguage(Long contentId, TranslateLanguage language) {
    queryFactory
      .delete(festivalTranslate)
      .where(
        festivalTranslate.contentId.eq(contentId)
          .and(festivalTranslate.language.eq(language))
      )
